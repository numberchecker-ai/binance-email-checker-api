# Binance Email Checker API — Bulk Verification | NumberChecker

The **Binance Email Checker API** accepts a text file of email addresses, creates an asynchronous bulk task, and exports a result file. This repository contains a runnable integration in eight languages.

- **Product page:** https://numberchecker.ai/products/binance-email-checker
- **API documentation:** https://docs.numberchecker.ai/binance-email-checker
- **API base URL:** `https://api.numberchecker.ai`
- **Authentication:** `X-API-Key`
- **Get an API key:** https://platform.numberchecker.ai

> [!NOTE]
> This repository covers the standalone product code `binance_email` only. Other product codes are outside its scope.

## Product facts

| API fact | Value |
|---|---|
| Product code | `binance_email` |
...
1. Submit the input file to `POST /v1/tasks` with `task_type=binance_email`.
...
Only `binance_email`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/binance-email-checker
- **Documentation:** https://docs.numberchecker.ai/binance-email-checker
