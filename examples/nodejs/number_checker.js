// Binance Email Checker API — Node.js 18+ example. Docs: https://docs.numberchecker.ai/binance-email-checker
...
const TASK_TYPE = "binance_email";
async function submitTask(path) { const form = new FormData(); form.append("file", new Blob([fs.readFileSync(path)]), "emails.txt"); form.append("task_type", TASK_TYPE);
...
const created = await submitTask("examples/emails.txt"); console.log("task_id:", created.task_id);
